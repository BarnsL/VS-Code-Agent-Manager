            ...
            switch (payload.type) {
                ...
                case "reassignStepAgent":
                    if (payload.ticketId && payload.stepId) {
                        await this.actions.reassignStepAgent(payload.ticketId, payload.stepId, payload.agentName);
                    }
                    return;
                case "spawnParallelLane":
                    if (payload.ticketId) {
                        await this.actions.spawnParallelLane(payload.ticketId, payload.agentName, payload.workflowResult);
                    }
                    return;
    ...
    renderHtml(webview, snapshot) {
        ...
        const queueMarkup = snapshot.queue.length
            ? snapshot.queue
                ...
                const commandType = item.status === "queued" ? "runTicketStep" : "focusTicket";
                return `
          ...
          <div class="column-body">
            ${tickets.length
                ? tickets
                    ...
                    const awaitingStep = ticket.steps.find((step) => step.status === "awaiting-output");
