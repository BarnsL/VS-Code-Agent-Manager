...
exports.analyzeStepOutput = analyzeStepOutput;
exports.buildStructuredHandoffPrompt = buildStructuredHandoffPrompt;
// ─── Queue button labels ─────────────────────────────────────────────────────
...
function shouldAutoProceedWorkflow(input) {
    if (!input.autoProceedEnabled)
        return false;
    if (!input.hasQueuedNextStep)
        return false;
    ...
    if (input.hasCapturedOutput === false)
        return false;
    return true;
}
...
function analyzeStepOutput(input) {
    ...
    const headline = `@${input.agentName} finished ${input.stepTitle}. ${keyPoints[0] ?? trimmed.slice(0, 200)}`.replace(/\s+/g, " ").trim();
...
function buildStructuredHandoffPrompt(input) {
    const priorSection = input.priorSteps.length
        ? input.priorSteps
            ...
            if (step.summary)
                parts.push(`**Handoff summary:** ${step.summary}`);
            if (step.analysis)
                parts.push(`**Manager analysis:**\n${step.analysis}`);
            if (step.output) {
