...
function buildTicketQuery(ticket: AgentTicket, step: WorkflowStep): string {
  ...
  const structured = buildStructuredHandoffPrompt({
...
function requestLikelyNeedsArtifacts(prompt: string): boolean {
  ...
  const implementationSignals =
    /(build|create|scaffold|implement|fix|refactor|code|app|component|api|endpoint|test)/.test(text);
  const nonArtifactSignals = /(brainstorm|research|analyze|analysis|plan|planning|review|summary)/.test(
    text
  );
  return implementationSignals && !nonArtifactSignals;
}

function extractArtifactPathsFromAnalysis(analysis: string | undefined): string[] {
  if (!analysis) return [];
...
function hasConcreteArtifactEvidence(ticket: AgentTicket): boolean {
...
function agentRequiresInteractiveChat(agent: AgentInfo | undefined): boolean {
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  async function launchTicketStep(ticketId: string): Promise<void> {
    ...
    if (started.ticket.autonomousMode && requiresInteractiveChat) {
      vscode.window.showInformationMessage(
        `@${started.step.agentName} uses MCP/browser tools, so this step is opening in Copilot Chat instead of autonomous LM mode.`
      );
    }
  ...
  async function submitStepOutput(
    ticketId: string,
    ...
    const active = ticket.steps.find(
      (step) => step.status === "active" || step.status === "awaiting-output"
    );
    if (!active) {
      vscode.window.showInformationMessage(
        "There’s no active step waiting for output. Run the next step first."
      );
      return;
    }
    ...
    if (!output) {
      vscode.window.showWarningMessage(
        "Paste the chat output for this step before submitting — the manager needs it to compose the next prompt."
      );
      return;
    }
    ...
    const lmAnalysis = await analyzeStepOutputWithLm({
      output,
      stepTitle: active.title,
      agentName: active.agentName,
    });
    if (lmAnalysis.kind === "analyzed") {
      analysisMarkdown = formatAnalysisAsMarkdown(lmAnalysis.analysis);
      analysisHeadline = lmAnalysis.analysis.headline;
    } else {
      const fallback = analyzeStepOutput({
        output,
        stepTitle: active.title,
        agentName: active.agentName,
      });
    ...
    if (plan.kind === "done") {
      ...
      if (needsArtifacts && !hasEvidence) {
        const verifier =
          tree.byName("verification-before-completion")?.name ??
        ...
        refreshAll();
        vscode.window.showWarningMessage(
          "Manager blocked auto-completion: implementation-style request had no concrete artifact evidence. Added a Completion Evidence Check step."
        );
      ...
      refreshAll();
      vscode.window.showInformationMessage(
        `Manager believes ${refreshed.title} is complete and is waiting for your acceptance.${plan.rationale ? ` Manager: ${plan.rationale}` : ""}`
      );
      return;
    }
    ...
    if (plan.kind === "no-model") {
      vscode.window.showWarningMessage(
        "Step output captured, but no Copilot language model is available to plan the next step. Use Reassign / Spawn Lane to drive manually, or sign in to Copilot Chat."
      );
      return;
    }
    ...
    if (plan.kind === "lm-error") {
      vscode.window.showErrorMessage(
        `Manager LM call failed: ${plan.error}. Step output is saved — retry from the dashboard when ready.`
      );
      return;
    }
    ...
    if (refreshed.continuousMode) {
      ...
      vscode.window.showInformationMessage(
        `Manager queued next step: ${plan.step.stepTitle ?? plan.step.agentName} (@${plan.step.agentName}). Click “Run Next Step” when ready.`
      );
    }
  }

  async function reassignStepAgent(
    ticketId: string,
    ...
    const agentName =
      ...
      (await vscode.window.showQuickPick(
        tree.getAll().map((agent) => ({ label: agent.name, description: agent.source })),
        { placeHolder: `Reassign ${step.title} (currently @${step.agentName})` }
      ))?.label;
    if (!agentName) return;
    ...
    refreshAll();
    vscode.window.showInformationMessage(
      `Reassigned ${step.title} → @${agentName}.`
    );
  }

  async function spawnParallelLane(
    ticketId: string,
    explicitAgent?: string,
    ...
    const agentName =
      ...
      (await vscode.window.showQuickPick(
        tree.getAll().map((agent) => ({ label: agent.name, description: agent.source })),
        { placeHolder: "Choose an agent for the parallel lane" }
      ))?.label;
    if (!agentName) return;
    ...
    await openChatQuery(
      fullPrompt,
      `Parallel lane prompt copied for @${agentName}`
    );
    await opsStore.recordAgentLaunch({
      agentName,
  ...
  async function setTicketSteering(
    ticketId: string,
  ...
  async function acceptTicketClosure(ticketId: string): Promise<void> {
  ...
  async function continueTicketWithSteering(ticketId: string): Promise<void> {
    const ticket = opsStore.findTicket(ticketId);
    if (!ticket) return;
    const steering = ticket.steeringNote?.trim();
    if (!steering) {
      vscode.window.showWarningMessage(
        "Add a steering note first, then continue the ticket."
      );
      return;
    }

    const nextAgent =
      routeTaskAgainstAvailableAgents(`${ticket.prompt}\n${steering}`, tree.getAll())[0]
  ...
  async function completeTicketStep(
    ...
    vscode.window.showWarningMessage(
      "Paste the chat output via 'Submit Output + Advance'. The manager will not mark a step done without it. Use 'Skip Step (Override)' if you really want to bypass."
    );
  }

  // Explicit override — only invoked when the user runs the Skip Step command.
  async function skipTicketStep(ticketId: string): Promise<void> {
    const ticket = opsStore.findTicket(ticketId);
    if (!ticket) return;
    const active = ticket.steps.find(
      (step) => step.status === "active" || step.status === "awaiting-output"
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "copilot-agents.submitStepOutput",
      async (ticketId?: string, output?: string) => {
        const ticket = ticketId
          ? opsStore.findTicket(ticketId)
          : await pickTicket((candidate) =>
              candidate.steps.some((step) => step.status === "active")
            );
        if (!ticket) return;
        const text =
          output ??
          (await vscode.window.showInputBox({
            ...
            validateInput: (value) =>
              value.trim() ? null : "Paste the chat output before submitting.",
          }));
        if (!text) return;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "copilot-agents.reassignStepAgent",
      async (ticketId?: string, stepId?: string, agentName?: string) => {
        const ticket = ticketId
          ? opsStore.findTicket(ticketId)
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "copilot-agents.spawnParallelLane",
      async (ticketId?: string, agentName?: string, prompt?: string) => {
        const ticket = ticketId
          ? opsStore.findTicket(ticketId)
          : await pickTicket((candidate) => candidate.status !== "done");
        if (!ticket) return;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "copilot-agents.setContinuousMode",
        ...
        const value =
          typeof enabled === "boolean"
            ...
            : (await vscode.window.showQuickPick(
                ...
                { placeHolder: `Continuous mode for ${ticket.title}` }
              ))?.value;
