...
function buildTicketQuery(ticket: AgentTicket, step: WorkflowStep): string {
  ...
  return buildStructuredHandoffPrompt({
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  async function submitStepOutput(
    ...
    if (ticket.continuousMode && nextQueuedStep) {
      ...
      vscode.window.showInformationMessage(
        `Step complete. Manager analysis stored. Click “Run Next Step” when you’re ready to launch @${nextQueuedStep.agentName}.`
      );
    } else {
      vscode.window.showInformationMessage(
        `Workflow complete for ticket ${ticket.title}.`
      );
    }
  }
  ...
  async function completeTicketStep(
    ...
    if (ticket.continuousMode) {
      ...
      if (hasNext) await launchTicketStep(ticket.id);
    }
  }
